...
async def ai_search_api(query: SearchQuery):
    ...
    if query.user_id not in graphs:
        graphs[query.user_id] = build_graph(
    ...
    last_known_rm = (
        state.values.get("last_rm_label", "UNKNOWN") if state.values else "UNKNOWN"
    )
    ...
    try:
        rm_label_result = await run_rm_labeller(
            pydantic_rm_labeller_model, rm_descriptions, query.query
        )
    ...
    response = answer_once(
        graph=graph, user_input=query.query, config=config, thread_id=query.user_id
    )
